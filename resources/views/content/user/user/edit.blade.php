        ...
        <div class="text-center p-0">
            <div class="card px-0 pt-4 pb-0 d-flex justify-content-center">
                ...
                <form id="user_from" action="{{ route('user.update', $user->id) }}" method="POST" enctype="multipart/form-data" novalidate> 
                  @csrf
                  <input type="hidden" name="selected_phone" id="selected_phone" value="{{$user->contact_number}}">
                  <input type="hidden" name="selected_country" id="selected_country" value="{{$user->countryCode}}">
                    <div class="row mx-3">
                                  ...
                                  <div class="form-floating">
                                      <select id="client_id" name="client_id" class="form-select">
                                          ...
                                          @foreach ($client as $key => $value)
                                              <option value="{{ $value->id }}" {{$value->id == $user->client_id ? 'selected' : ''}}>{{ $value->client_name }}
                                              </option>
                                          @endforeach
                                      </select>
...
<script>
  $("#countryCode").val($("#selected_country").val());
  $("#contact_number").val($("#selected_phone").val());
</script>
@endsection
